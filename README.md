...
Copy `backend/.env.example` to `backend/.env` and adjust the values for your local environment.
...
The frontend can optionally use `frontend/.env.local.example` as a starting point for API configuration.
...
- The staged improvement plan is tracked in `docs/improvement-roadmap.md`.
